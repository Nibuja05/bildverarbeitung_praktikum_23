...
class SIFT_Algorithm:
    ...
    def create_scale_space(u_in, sift_params):
        ...
        R = E.resize(
            ...
            fx=J / B.delta_min,
            fy=J / B.delta_min,
            interpolation=E.INTER_LINEAR,
        )
        G = A.sqrt(B.sigma_min**2 - B.sigma_in**2) / B.delta_min
        S = E.GaussianBlur(R, (0, 0), G)
        M = [S]
        for F in D(1, B.n_spo + 3):
            H = B.sigma_min * O(2.0, F / B.n_spo)
            # print(H, C[0][F - 1])
            G = A.sqrt(H**2 - C[0][F - 1] ** 2) / B.delta_min
            # print(G)
            C[0].append(H)
            M.append(E.GaussianBlur(M[F - 1], (0, 0), G))
        # print(C[0])
        K.append(M)
        for I in D(1, B.n_oct):
            ...
            L.append(N)
            P = K[I - 1][B.n_spo - 1]
            C.append([C[I - 1][B.n_spo - 1]])
            P = E.resize(P, (0, 0), fx=0.5, fy=0.5, interpolation=E.INTER_LINEAR)
            Q = [P]
            for F in D(1, B.n_spo + 3):
                H = N / B.delta_min * B.sigma_min * O(2.0, F / B.n_spo)
                G = A.sqrt(H**2 - C[I][F - 1] ** 2) / N
                C[I].append(H)
                Q.append(E.GaussianBlur(Q[F - 1], (0, 0), G))
            K.append(Q)
        return K, L, C
    ...
    def create_dogs(scale_space, sift_params):
        ...
        for H in D(0, A.n_oct):
            C = scale_space[H]
            F = []
            for G in D(A.n_spo + 2):
                F.append(E.subtract(C[G + 1], C[G]))
            B.append(F)
        return B
            ...
            for G in D(1, J.n_spo + 1):
                ...
                for B in D(1, H.shape[0] - 1):
                    for C in D(1, H.shape[1] - 1):
                        ...
                        E = A.delete(E, 4)
                        E = A.append(E, N[B - 1 : B + 2, C - 1 : C + 2].flatten())
                        E = A.append(E, O[B - 1 : B + 2, C - 1 : C + 2].flatten())
                        if A.max(E) < H[B, C] or A.min(E) > H[B, C]:
                            L.append(
                                SIFT_KeyPoint(
                                    F, G, C, B, sigmas[F][G], K[F] * C, K[F] * B
                                )
                            )
        return L
    ...
    def taylor_expansion(
        ...
        for extremum in extremas:
            ...
            if (
                abs(dog_scales[extremum.o][extremum.s][extremum.n, extremum.m])
                < 0.8 * sift_params.C_DoG
            ):
                continue
            ...
            for _ in range(5):
                ...
                g_o_smn = A.matrix(
                    [
                        [
                            next_scale[n, m] - previous[n, m],
                            current[n, m + 1] - current[n, m - 1],
                            current[n + 1, m] - current[n - 1, m],
                        ]
                    ]
                ).T
                ...
                hessian = A.matrix([[h11, h12, h13], [h12, h22, h23], [h13, h23, h33]])
                ...
                if A.max(A.abs(alpha)) < 0.6:
                    ...
                    omega = current[extremum.n, extremum.m] + 0.5 * (
                    ...
                    new_extremum = SIFT_KeyPoint(
                        extremum.o, s, m, n, sigma, x, y, omega
                    )
    ...
    def filter_extremas(
        ...
        for extremum in extremas:
            ...
            hessian = A.matrix([[h11, h12], [h12, h22]])
    ...
    @staticmethod
    def gradient_2d(
        scale_space: list[list[A.ndarray]], sift_params: SIFT_Params
            ...
            if (
                ...
                for m in range(
                    ...
                    for n in range(
                        ...
                        weight: float = A.exp(
                            (-(sM * sM + sN * sN)) / (2 * (sift_params.lambda_ori**2))
                        )
                        # and angle
                        angle = SIFT_Algorithm._float_modulo(
                            A.arctan2(delta_m, delta_n), 2 * A.pi
                        )
                        # calculate histogram index
                        if angle < 0:
                            angle += 2 * A.pi
                        b_index = (
                            int(angle / (2 * A.pi) * sift_params.n_bins + 0.5)
                            % sift_params.n_bins
                        )
                ...
                for k in range(sift_params.n_bins):
                    ...
                    if (
                        h_k[k] > prev_hist
                        and h_k[k] > next_hist
                        and h_k[k] >= sift_params.t * max_h_k
                    ):
                        # calculate offset
    ...
    def create_descriptors(
        ...
        for keypoint in keypoints:
            ...
            # relative patch size
            relative_patch_size = (
                (1 + 1 / sift_params.n_hist) * sift_params.lambda_descr * key_sigma
            )
            ...
            if (
                ...
                histograms: list[list[A.ndarray]] = [
                    [A.zeros(sift_params.n_ori) for _ in range(sift_params.n_hist)]
                ...
                for m in range(
                    ...
                    for n in range(
                        ...
                        x_vedge_mn = A.cos(-keypoint.theta) * (m - key_x) - A.sin(
                            -keypoint.theta
                        ) * (n - key_y)
                        y_vedge_mn = A.sin(-keypoint.theta) * (m - key_x) + A.cos(
                            -keypoint.theta
                        ) * (n - key_y)
                        ...
                        if max(abs(x_vedge_mn), abs(y_vedge_mn)) < relative_patch_size:
                            ...
                            theta_mn = SIFT_Algorithm._float_modulo(
                                (A.arctan2(delta_m, delta_n) - keypoint.theta), 2 * A.pi
                            )
                            magnitude: float = A.sqrt(
                                delta_m * delta_m + delta_n * delta_n
                            )
                            weight: float = A.exp(
                                -(x_vedge_mn * x_vedge_mn + y_vedge_mn * y_vedge_mn)
                                / (2 * (sift_params.lambda_descr * key_sigma) ** 2)
                            )
